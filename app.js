...
/* ===========================
   🧩 Formato personalizado por proyecto (DOCX)
=========================== */
async function refreshUserTemplateStatus(){
  const p = getActiveProject();
  if (!p){
    if ($("userTplStatus")) $("userTplStatus").textContent = "—";
    const cb = $("useUserTpl");
  ...
  try{
    ...
    if (hasDocx){
      ...
      parts.push(`Word: ${name} · ${slots||"?"} foto(s) por bloque`);
    }
    if (hasPdf){
      const pslots = Number(tpl.pdfSlots || 0);
      const pname = tpl.pdfName || "Plantilla.pdf";
      parts.push(`PDF: ${pname} · ${pslots||"?"} slot(s)`);
    }
...
$('btnUserTplPreview').onclick = async () => {
  ...
  try{
    const tpl = await tplGet(p.id);
    if (!tpl){ alert('Este proyecto no tiene formato cargado.'); return; }

    const lines = [`Proyecto: ${p.name}`];

    if (tpl.ab){
      const meta = await parseUserTemplateMetaFromArrayBuffer(tpl.ab);
      lines.push(`Word: ${tpl.name || 'Plantilla.docx'}`);
      lines.push(`  Bloque repetible: ${meta.hasStart && meta.hasEnd ? 'OK' : 'FALTA'}`);
      lines.push(`  Foto(s) por bloque detectadas: ${meta.slots || 0}`);
    } else {
      lines.push('Word: (no cargado)');
    }

    if (tpl.pdfAb){
      const pmeta = await parseUserPdfTemplateMetaFromArrayBuffer(tpl.pdfAb);
      lines.push(`PDF: ${tpl.pdfName || 'Plantilla.pdf'}`);
      lines.push(`  Campos detectados: ${pmeta.fieldCount || 0}`);
      lines.push(`  Slots de foto detectados: ${pmeta.slots || 0}`);
      lines.push('');
      lines.push('Convención recomendada (PDF):');
      lines.push('  LOGI_PROYECTO, LOGI_PERIODO, LOGI_FECHA_DESDE, LOGI_FECHA_HASTA (opcionales)');
      lines.push('  LOGI_ITEM, LOGI_ITEM_DESC, LOGI_ITEM_UNIDAD (opcionales)');
      lines.push('  LOGI_FOTO_1..N  y opcional LOGI_FOTO_1_DESC..N_DESC');
    } else {
      lines.push('PDF: (no cargado)');
    }

    lines.push('');
    lines.push('Tip: en Word, los slots de fotos van en ALT TEXT de cada imagen con:');
    lines.push('  [[LOGI:FOTO_1]], [[LOGI:FOTO_2]] ...');
    lines.push('y debe existir [[LOGI:ITEMS_START]] y [[LOGI:ITEMS_END]].');
...
$("userTemplateInput").onchange = async () => {
  ...
  $("userTplStatus").textContent = "Leyendo plantilla…";
  try{
    const ab = await file.arrayBuffer();
    const nameLower = String(file.name || "").toLowerCase();
    const isPdf = nameLower.endsWith(".pdf") || (String(file.type||"").includes("pdf"));

    const prev = (await tplGet(p.id)) || {};

    if (isPdf){
      const meta = await parseUserPdfTemplateMetaFromArrayBuffer(ab);
      if (!meta.slots) throw new Error("La plantilla PDF no tiene slots (campos) LOGI_FOTO_1..N.");
      await tplPut(p.id, { ...prev, pdfName: file.name, pdfAb: ab, pdfSlots: meta.slots });
      await refreshUserTemplateStatus();
      alert("Formato PDF cargado ✅\n\nTip: crea campos en el PDF (AcroForm) con nombres LOGI_FOTO_1..N para ubicar las fotos.\nOpcional: LOGI_FOTO_1_DESC..N_DESC para la descripción.");
    } else {
      const meta = await parseUserTemplateMetaFromArrayBuffer(ab);
      if (!meta.hasStart || !meta.hasEnd){
        throw new Error("La plantilla no trae los marcadores [[LOGI:ITEMS_START]] y [[LOGI:ITEMS_END]].");
      }
      if (!meta.slots) throw new Error("La plantilla no tiene slots [[LOGI:FOTO_n]].");

      await tplPut(p.id, { ...prev, name: file.name, ab, slots: meta.slots });
      await refreshUserTemplateStatus();
      alert("Formato Word cargado ✅\n\nTip: cuando exportes Word, Logi usará esta plantilla automáticamente.");
    }
  }catch(e){
    console.error(e);
...
// =========================
// 🗓️ Formatos auxiliares (plantillas de usuario)
// =========================
...
// =========================
// 🔢 Ordenamiento por ítem (para export)
// - Ítems numéricos primero (2.1, 10.3, ...)
// - Luego ítems no numéricos (A-01, 3.1A, ...)
// - Al final: sin ítem
// Dentro de cada ítem: por createdAt ascendente
// =========================
function _itemCodeKey(code){
  const s = String(code || "").trim();
  if (!s) return { kind: 2, raw: "" };

  // numérico puro (p. ej. 3, 3.1, 10.2.5)
  if (/^\d+(?:\.\d+)*$/.test(s)){
    const parts = s.split(".").map(x => parseInt(x,10)).filter(n => Number.isFinite(n));
    return { kind: 0, parts, raw: s };
  }

  // mixto que comienza con numérico (p. ej. 3.1A)
  const m = s.match(/^(\d+(?:\.\d+)*)(.*)$/);
  if (m){
...
function compareItemCodes(a, b){
  ...
  // no numéricos
  if (A.kind === 1){
    // si ambos tienen prefijo numérico, compara primero esas partes
    if (A.parts && B.parts){
      const la = A.parts.length, lb = B.parts.length;
      const L = Math.max(la, lb);
      for (let i=0; i<L; i++){
...
function buildDocxCaption(n, meta, templateId){
  ...
  // Si no hay ítem asignado, NO mostrar "SIN ASIGNAR"; dejar el campo vacío.
  // Para el caption estándar, simplemente omitimos el segmento de ítem.
...
async function makeBlankJpegBlob(w=20,h=20,quality=0.85){
  // Crea una imagen blanca pequeña (se estira al tamaño del frame en el DOCX)
...
/* ===========================
   ...
   Convención de campos (recomendado):
   - Texto global: LOGI_PROYECTO, LOGI_PERIODO, LOGI_FECHA_DESDE, LOGI_FECHA_HASTA, LOGI_FECHA_INICIO, LOGI_FECHA_FIN
   - Texto por ítem: LOGI_ITEM, LOGI_ITEM_DESC, LOGI_ITEM_UNIDAD, LOGI_ITEM_CANT (opcional)
   - Slots de fotos: LOGI_FOTO_1 .. LOGI_FOTO_N
     y opcional captions: LOGI_FOTO_1_DESC .. LOGI_FOTO_N_DESC
   Nota: también aceptamos nombres con el tag visible: [[LOGI:FOTO_1]] etc.
=========================== */
function _pdfNormName(n){
  return String(n||"").trim();
}
function _pdfMatchSlotName(name){
  const n = _pdfNormName(name);
  let m = n.match(/^LOGI_FOTO_(\d+)$/i);
  if (m) return parseInt(m[1],10);
  m = n.match(/^\[\[LOGI:FOTO_(\d+)\]\]$/i);
  if (m) return parseInt(m[1],10);
  return null;
}
function _pdfMatchSlotDescName(name){
  const n = _pdfNormName(name);
  let m = n.match(/^LOGI_FOTO_(\d+)_DESC$/i);
  if (m) return parseInt(m[1],10);
  m = n.match(/^\[\[LOGI:FOTO_(\d+)_DESC\]\]$/i);
  if (m) return parseInt(m[1],10);
  return null;
}
function _pdfFieldGetAny(form, names){
  for (const nm of names){
    try{
      const tf = form.getTextField(nm);
      if (tf) return tf;
    }catch{}
  }
  return null;
}
function _pdfFieldHasAny(form, names){
  for (const nm of names){
    try{ form.getField(nm); return nm; }catch{}
  }
  return null;
}

async function parseUserPdfTemplateMetaFromArrayBuffer(ab){
  ...
  for (const n of fields){
    const k = _pdfMatchSlotName(n);
    if (k != null) slotNums.push(k);
  }
  const slots = slotNums.length ? Math.max(...slotNums) : 0;
  return { slots, fieldCount: fields.length, fields };
}
...
async function buildRegistroFotograficoPdfFromUserTemplateBlob(selected, tituloProyecto, startISO, endISO, tplRec, opts={}){
  ...
  // Orden y agrupación: ítems ascendente, y al final sin ítem. Dentro del ítem: createdAt asc.
  ...
  if (!slots){
    const meta = await parseUserPdfTemplateMetaFromArrayBuffer(tplRec.pdfAb);
    slots = meta.slots;
  }
  if (!slots) throw new Error("La plantilla PDF no tiene campos LOGI_FOTO_n (slots de fotos).");

  // Total páginas (por chunks de slots)
  let totalPages = 0;
  for (const code of order){
    totalPages += Math.ceil((groups[code].length || 0) / slots);
  }

  const out = await PDFDocument.create();

  const periodo = formatPeriodoUpperES(startISO || "", endISO || "");
  const fechaInicioL = formatDateLongUpperES(startISO || "");
  const fechaFinL = formatDateLongUpperES(endISO || "");

  let donePages = 0;

  for (const code of order){
    const arr = groups[code] || [];
    const itemDesc = (code && (catalogMap[code] || arr[0]?.itemDesc)) ? String(catalogMap[code] || arr[0]?.itemDesc || "").trim() : "";
    const itemUnit = (code && catalogUnitMap[code]) ? String(catalogUnitMap[code] || "").trim() : "";

    for (let off = 0; off < arr.length; off += slots){
      const chunk = arr.slice(off, off + slots);

      // Cargar PDF base por página (evita colisiones de nombres de campos al duplicar)
      const basePdf = await PDFDocument.load(tplRec.pdfAb);
      let form = null;
      try{ form = basePdf.getForm(); }catch{ form = null; }

      // Fill textos (si existen)
      if (form){
        const tfProj = _pdfFieldGetAny(form, ["LOGI_PROYECTO", "[[LOGI:PROYECTO]]"]);
        if (tfProj) try{ tfProj.setText(tituloProyecto || ""); }catch{}

        const tfPer = _pdfFieldGetAny(form, ["LOGI_PERIODO", "[[LOGI:PERIODO]]"]);
        if (tfPer) try{ tfPer.setText(periodo || ""); }catch{}

        const tfFD = _pdfFieldGetAny(form, ["LOGI_FECHA_DESDE", "[[LOGI:FECHA_DESDE]]"]);
        if (tfFD) try{ tfFD.setText(startISO || ""); }catch{}

        const tfFH = _pdfFieldGetAny(form, ["LOGI_FECHA_HASTA", "[[LOGI:FECHA_HASTA]]"]);
        if (tfFH) try{ tfFH.setText(endISO || ""); }catch{}

        const tfFI = _pdfFieldGetAny(form, ["LOGI_FECHA_INICIO", "[[LOGI:FECHA_INICIO]]"]);
        if (tfFI) try{ tfFI.setText(fechaInicioL || ""); }catch{}

        const tfFF = _pdfFieldGetAny(form, ["LOGI_FECHA_FIN", "[[LOGI:FECHA_FIN]]"]);
        if (tfFF) try{ tfFF.setText(fechaFinL || ""); }catch{}

        const tfItem = _pdfFieldGetAny(form, ["LOGI_ITEM", "[[LOGI:ITEM]]"]);
        if (tfItem) try{ tfItem.setText(code || ""); }catch{}

        const tfItemD = _pdfFieldGetAny(form, ["LOGI_ITEM_DESC", "LOGI_DESC", "[[LOGI:DESC]]", "[[LOGI:ITEM_DESC]]"]);
        if (tfItemD) try{ tfItemD.setText(itemDesc || ""); }catch{}

        const tfUnit = _pdfFieldGetAny(form, ["LOGI_ITEM_UNIDAD", "LOGI_UNIDAD", "[[LOGI:UNIDAD]]", "[[LOGI:ITEM_UNIDAD]]"]);
        if (tfUnit) try{ tfUnit.setText(itemUnit || ""); }catch{}

        const tfCant = _pdfFieldGetAny(form, ["LOGI_ITEM_CANT", "LOGI_CANT", "[[LOGI:CANT]]", "[[LOGI:ITEM_CANT]]"]);
        if (tfCant) try{ tfCant.setText(""); }catch{}
      }

      // Render fotos sobre los rectángulos de los campos
      const page = basePdf.getPages()[0];
      const pad = 2;

      for (let i = 0; i < slots; i++){
        const n = i + 1;
        const rec = chunk[i] || null;

        const nameA = `LOGI_FOTO_${n}`;
        const nameB = `[[LOGI:FOTO_${n}]]`;
        let fieldName = null;
        if (form){
          fieldName = _pdfFieldHasAny(form, [nameA, nameB]);
        }

        if (rec && fieldName && form){
          try{
            const f = form.getField(fieldName);
            const widgets = (typeof f.getWidgets === "function") ? f.getWidgets() : [];
            const w0 = widgets && widgets[0] ? widgets[0] : null;
            if (w0 && typeof w0.getRectangle === "function"){
              ...
              let imgBlob = rec.blob;
              try{ imgBlob = await normalizeToFixedFrameJpg(imgBlob, boxW, boxH, 0.9, "contain"); }catch{}
              const abImg = await imgBlob.arrayBuffer();
              const u8 = new Uint8Array(abImg);
              const isPng = (u8[0] === 0x89 && u8[1] === 0x50 && u8[2] === 0x4E && u8[3] === 0x47);
              const img = isPng ? await basePdf.embedPng(u8) : await basePdf.embedJpg(u8);

              const scale = Math.min(boxW / img.width, boxH / img.height);
              const drawW = img.width * scale;
              const drawH = img.height * scale;
              const xImg = boxX + (boxW - drawW)/2;
              const yImg = boxY + (boxH - drawH)/2;

              page.drawImage(img, { x: xImg, y: yImg, width: drawW, height: drawH });
            }
          }catch(e){
            console.warn("Slot foto no pude dibujar:", n, e);
          }
        }

        if (rec && form){
          const dn = `LOGI_FOTO_${n}_DESC`;
          const dn2 = `[[LOGI:FOTO_${n}_DESC]]`;
          const capField = _pdfFieldGetAny(form, [dn, dn2]);
          if (capField){
            try{ capField.setText(String(rec.descripcion || "").trim()); }catch{}
          }
        }
      }
      ...
      const [copied] = await out.copyPages(basePdf, [0]);
      out.addPage(copied);

      donePages++;
      if (onProgress) await onProgress(donePages, totalPages);
...
function _findPrev(doc, idx, needle){
  // OJO: "<w:p" también matchea "<w:pPr" (propiedades de párrafo) y eso rompe el slicing.
  // Para párrafos, buscamos SOLO inicios reales: "<w:p>" o "<w:p ...>".
  if (needle === "<w:p"){
...
async function buildRegistroFotograficoDocxFromUserTemplateBuffer(selected, tituloProyecto, startISO, endISO, tplRec, opts={}){
  ...
  // Agrupar por ítem (código) ordenando: ítems ascendente, y al final las fotos sin ítem.
  // Dentro de cada ítem: createdAt ascendente.
  const photos = sortPhotosByItemThenTime(selected || []);
  const groups = {};
  const order = [];
  for (const it of photos){
    // Si no hay ítem asignado, lo agrupamos como cadena vacía y NO mostramos "SIN ASIGNAR" en el export.
    const code = String(it.itemCode || "").trim();
    if (!groups[code]){ groups[code] = []; order.push(code); }
    groups[code].push(it);
  }
  ...
  // Caso A (ideal): los marcadores están fuera de la tabla del bloque.
  // Caso B (común en plantillas hechas por usuarios): los marcadores quedaron DENTRO de la tabla;
  ...
  if (sIdx < 0 || eIdx < 0 || eIdx <= sIdx) throw new Error("No encontré [[LOGI:ITEMS_START]]/[[LOGI:ITEMS_END]] en la plantilla.");
  ...
  // ¿Los marcadores están dentro de una misma tabla?
  ...
  if (markersInsideSameTbl){
    ...
    suffix = docXml.slice(eP1 + "</w:p>".length);
	  ...
	  // Evitar corrupción por IDs duplicados al repetir el bloque:
	  ...
	  let maxDocPr = docPrNums.length ? Math.max(...docPrNums) : 0;
		  ...
		  // También: algunos DOCX (especialmente hechos en Word) reutilizan pic:cNvPr id="0".
  ...
  // Relación e imagen en blanco (para slots sin foto; evita que se cuele la foto placeholder del formato)
  ...
  for (const code of order){
    ...
    while (idx < arr.length){
      ...
      idx += slots;
	      ...
	      bx = bx.replace(/<wp:docPr([^>]*?)id=\"(\d+)\"([^>]*?)\/>/g, (m, a1, _old, a2) => {
	        ...
	        return `<wp:docPr${attrs}/>`;
		      ...
		      // Renumerar pic:cNvPr id (Word también exige unicidad en estos IDs)
      ...
      // Si no hay ítem asignado, dejamos el campo vacío.
      ...
      for (let s = 1; s <= slots; s++){
        ...
        if (baseRid){
          if (it.blob){
            ...
            // Si por alguna razón no hay blob, dejamos el slot en blanco
            bx = bx.replaceAll(baseRid, blankRid);
          }
        }
  ...
  // Ensamblar doc final
  // Separador mínimo entre bloques para evitar que Word intente "pegar" tablas consecutivas.
...
async function exportExpressByMode(modo, desde, hasta, fmt) {
  ...
  if (onlyDone) selected = selected.filter(x => !!x.done);
    ...
    // Orden para export: por ítem ascendente y al final las fotos sin ítem
    // (Dentro de cada ítem: por createdAt asc)
    ...
    if (fmt === "docx") {
      // Si hay formato personalizado (plantilla DOCX) para este proyecto, úsalo SOLO si el usuario lo pidió
      const activeId = getActiveProjectId() || ensureProjects().activeId;
...
// PDF
const activeIdPdf = getActiveProjectId() || ensureProjects().activeId;
const tplRecPdf = useUserTpl ? await tplGet(activeIdPdf) : null;
if (tplRecPdf && tplRecPdf.pdfAb){
  const zi = $("zipInfo");
  const nextPaint = () => new Promise(res => requestAnimationFrame(() => setTimeout(res, 0)));
  if (zi){ zi.textContent = `Generando PDF (formato del usuario)... 0/?`; await nextPaint(); }

  const onProgress = async (done, tot) => {
    if (!zi) return;
    zi.textContent = `Generando PDF (formato del usuario)... ${done}/${tot}`;
    await nextPaint();
  };

  const pdfBlobUser = await buildRegistroFotograficoPdfFromUserTemplateBlob(
    selected, tituloProyecto, start, end, tplRecPdf,
    { onProgress }
  );
  if (!pdfBlobUser) return;

  const itemTag = itemFilter ? `_ITEM_${sanitizeName(itemFilter)}` : "";
  await shareBlobAsFile(pdfBlobUser, `Logi_Reporte_${start}_a_${end}${itemTag}.pdf`, "application/pdf", "Logi - PDF");
